...
from agents.inquiry import run_episode, inquiry_from_stage
...
class UserInterface():
    ...
    def run_env(self) -> None:
        ...
        cl_inferential_theory = random_inferential_theory_generator(msf=msf, for_move_size='random', against_move_size='random', for_move_chance=0.5, against_move_chance=0.5)
        cr_inferential_theory = random_inferential_theory_generator(msf=msf, for_move_size='random', against_move_size='random', for_move_chance=0.5, against_move_chance=0.5)

        inq = run_episode(frame=msf, target='random', proposal='undeclared', cl_strategy='one step ahead', cr_strategy='minimize ac', cl_inferential_theory=cl_inferential_theory, cr_inferential_theory=cr_inferential_theory)
    ...
    def run_env_manual(self) -> None:
            ...
            inq = run_single_inquiry_for(frame=msf, target=target, proposal=proposal, cl_strategy=cl_strat, cr_strategy=cr_strat, cl_inferential_theory=cl_inferential_theory, cr_inferential_theory=cr_inferential_theory)
